...
```mermaid
flowchart TD
    A["Подготовка и валидация входных данных"] --> B["Техническое редактирование"]
    B --> C["Литературное редактирование"]
    
    C --> D["Корректорская правка"]
    
    D --> E["Итоговая оценка и унификация изменений"]
...
```mermaid
flowchart TD
    A["Входящие переведённые данные"] --> B["Разделение на сегменты и анализ"]
    B --> C["Параллельные потоки проверки"]
    
    C --> D["Применение корректорских правил"]
    D --> E["Применение литературных правил"]
    E --> F["Применение технических правил"]
    
    F --> G["Разрешение конфликтов и слияние"]
    G --> H["Сохранение цветовой маркировки"]
